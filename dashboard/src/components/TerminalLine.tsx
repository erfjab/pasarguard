...
export function TerminalLine({ log, noTimestamp, searchTerm }: LogLineProps) {
    ...
    return (
        <div
            className={cn(
                "font-mono text-[10px] sm:text-xs flex flex-row gap-2 sm:gap-3 py-1 sm:py-0.5 group",
                type === "error"
        ...
        >
            <div className="flex items-start gap-x-2">
                ...
                {!noTimestamp && (
                    <span className="select-none pl-1 sm:pl-2 text-[9px] sm:text-xs text-muted-foreground w-16 sm:w-20 flex-shrink-0">
                        {formattedTime}
                    </span>
                )}
                ...
                <Badge
                    variant={variant}
                    className={cn("w-14 justify-center text-[10px] px-1 py-0",locale==="fa" && "font-body")}
                >
                    {t(`nodes.logs.${type}`)}
                </Badge>
            </div>
